  ...
  return (
    <>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top bar — Discord style: thin, channel name + actions */}
        <header className="h-12 shrink-0 flex items-center justify-between px-4 border-b border-[#1e1f22] bg-[#313338]">
          ...
          <div className="flex items-center gap-2">
            <Link href={`/builder/${formId}`} className="px-3 py-1 rounded text-xs text-[#b5bac1] hover:bg-[#3f4147] hover:text-[#f2f3f5] transition-colors">
              Open Builder
            </Link>
            <button onClick={exportCSV} className="px-3 py-1 rounded text-xs text-[#b5bac1] hover:bg-[#3f4147] hover:text-[#f2f3f5] transition-colors">
              Export CSV
            </button>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto bg-[#313338] p-6">
          <div className="max-w-7xl mx-auto space-y-4">
            ...
            <div ref={overviewRef} className="grid grid-cols-3 gap-3">
              {[
                { label: "Total Responses", value: String(overview?.totalResponses ?? 0) },
                { label: "Completion Rate", value: `${overview?.completionRate ?? 0}%` },
                { label: "Total Views", value: String(overview?.views ?? 0) },
              ].map(({ label, value }) => (
                <div key={label} className="rounded-lg p-4 bg-[#2b2d31]">
                  <span className="text-xs text-[#949ba4]">{label}</span>
                  <p className="text-2xl font-bold text-[#f2f3f5] mt-1">{value}</p>
                </div>
              ))}
            </div>

            {/* Charts */}
            <div ref={breakdownRef} className="grid grid-cols-3 gap-3">
              {/* Bar Chart */}
              <div className="col-span-2 rounded-lg p-5 bg-[#2b2d31]">
              ...
              {/* Donut */}
              <div className="rounded-lg p-5 bg-[#2b2d31] flex flex-col">
            ...
            {/* Responses Table */}
            <div ref={responsesRef} className="rounded-lg overflow-hidden bg-[#2b2d31]">
