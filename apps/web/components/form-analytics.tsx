...
import { BarChart2, Eye, Loader2, Users } from "lucide-react";
    ...
    <main className="flex-1 overflow-y-auto bg-[#313338] p-6">
      <div className="max-w-5xl">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-[#f2f3f5]">{form?.title ?? "Analytics"}</h1>
            <p className="text-sm text-[#949ba4] mt-1">Response analytics and insights</p>
          </div>
        ...
        {timeline && timeline.length > 0 && (
          <div className="rounded-lg p-4 bg-[#2b2d31] mb-6">
            <p className="text-sm font-semibold text-[#f2f3f5] mb-4">Response Timeline</p>
            <div className="flex items-end gap-1 h-32">
              {timeline.map((day, i) => {
                const maxCount = Math.max(...timeline.map(d => d.count));
                const height = maxCount > 0 ? (day.count / maxCount) * 100 : 0;
                return <div key={i} className="flex-1 rounded-sm bg-[#5865f2] hover:bg-[#4752c4] transition-colors" style={{ height: `${Math.max(height, 4)}%` }} />;
              })}
            </div>
          </div>
        )}
        ...
        <div className="rounded-lg overflow-hidden bg-[#2b2d31]">
          ...
          {!responses?.responses.length ? (
            ...
            <table className="w-full">
              <thead><tr className="border-t border-[#3f4147]"><th className="px-4 py-2 text-left text-[11px] font-semibold uppercase text-[#949ba4]">Email</th><th className="px-4 py-2 text-left text-[11px] font-semibold uppercase text-[#949ba4]">Submitted</th></tr></thead>
              <tbody>
